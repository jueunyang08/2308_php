<template>
 <div class="header">
    <ul>
      <li v-if="$store.state.flgTapUI !== 0" class="header-button header-button-left" @click="move">취소</li>
      <li><img class="logo" alt="Vue logo" src="./assets/logo.png"></li>
      <li v-if="$store.state.flgTapUI === 1"
      @click="addBoard()"
...
<!-- <p>{{ $store.state.phptest }}</p> -->
  ...
  <!-- 더보기 버튼  -->
<button v-if="$store.state.flgBtnMoreView && $store.state.flgTapUI === 0" @click="listMore()">더보기</button>
